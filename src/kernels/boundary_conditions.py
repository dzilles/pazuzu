...
BC_NO_SLIP_WALL = 8      # No-Slip Wall (Navier-Stokes)
BC_ISOTHERMAL_WALL = 9   # No-Slip Wall with fixed Temperature
...
@wp.func
def apply_no_slip_wall(q_inner: Any, params: Any):
    """
...
def apply_isothermal_wall(q_inner: Any, T_wall: Any, params: Any):
    """
    Applies No-Slip Wall boundary condition with fixed temperature.
    """
    rho = q_inner[0]
    # Target p = rho * R * T_wall
    p_ghost = rho * params.gas_constant * T_wall
    E_ghost = p_ghost / (params.gamma - params.one) + params.half * (q_inner[1]*q_inner[1] + q_inner[2]*q_inner[2]) / rho
