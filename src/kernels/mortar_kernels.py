...
def compute_mortar_fluxes(
    ...
    for k in range(N1):
        ...
        f_int_f = zero_v
        if face == 1 or face == 3:
            f_int_f = euler.flux_x(q_f, params)
        else:
            f_int_f = euler.flux_y(q_f, params)
        ...
        if (face == 1) or (face == 2):
            dg_val_f = dg_R # Faces 1 and 2 (+x and +y)
        ...
        dg_val_c = dg_L
        if (coarse_face == 1) or (coarse_face == 2):
            dg_val_c = dg_R
        
        for m in range(N1):
            weight_r = R_left[m, k]
            if subface == 1:
                weight_r = R_right[m, k]
    ...
    for m in range(N1):
        ...
        f_int_c_m = zero_v
        if face == 1 or face == 3:
            f_int_c_m = euler.flux_x(q_c_m, params)
        else:
            f_int_c_m = euler.flux_y(q_c_m, params)
