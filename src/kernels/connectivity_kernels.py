...
def mark_mortar_neighbors(
    ...
    # Opposing face in B-R-T-L ordering:

    # 0 (Bottom) <-> 2 (Top)

    # 1 (Right)  <-> 3 (Left)

    coarse_face = (fine_face + 2) % 4
...
def compute_neighbors(
    ...
    # Check 4 directions in B-R-T-L Order:

    # 0: Bottom (-y), 1: Right (+x), 2: Top (+y), 3: Left (-x)
        ...
        if n_idx == -1 and level > 0 and valid_n == 1:
            ...
            if n_idx != -1:
                ...
                if m_idx < max_mortars:
                    ...
                    # Bottom/Top (0/2) -> subface depends on x (ix & 1)

                    # Right/Left (1/3) -> subface depends on y (iy & 1)
