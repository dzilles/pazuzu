...
def generate_mesh(filename):
    gmsh.initialize()
    gmsh.model.add("cylinder_ns")

    lc = 0.4
    lc_cyl = 0.05 # Finer near cylinder for NS

    # Domain Points
    p1 = gmsh.model.geo.addPoint(-5, -5, 0, lc)
    p2 = gmsh.model.geo.addPoint(15, -5, 0, lc) # Longer wake
    p3 = gmsh.model.geo.addPoint(15, 5, 0, lc)
    ...
    # Cylinder (Radius 0.5) centered at (0,0)
    R = 0.5
    ...
    gmsh.model.mesh.field.setNumbers(f1, "CurvesList", [arc1, arc2, arc3, arc4])
    
    f2 = gmsh.model.mesh.field.add("Threshold")
    gmsh.model.mesh.field.setNumber(f2, "IField", f1)
    gmsh.model.mesh.field.setNumber(f2, "LcMin", lc_cyl)
    gmsh.model.mesh.field.setNumber(f2, "LcMax", lc)
    gmsh.model.mesh.field.setNumber(f2, "DistMin", 0.6)
    gmsh.model.mesh.field.setNumber(f2, "DistMax", 2.0)
